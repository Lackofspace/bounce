...
public:
    float width, height, dx, dy, speed = 0;                            // высота и ширина, ускорение (по OX и по OY), скорость движения мяча
    ...
    enum class stateObject {                                        // добавляем перечисляемый тип (с областью видимости) - состояние объекта
        left,
        right,
        up,
        down,
    ...
    /// Класс текстуры, использует изображение для рисования.
    ...
    */
    ...
    void
    update(float time){                                           // для двигающейся картинки
